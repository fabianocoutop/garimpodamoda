...
function renderizarVitrine(produtos) {
    ...
    paginatedProducts.forEach(peça => {
        ...
        if (!peça.disponivel) {
            ...
            btnHtml = `
            <button class="add-to-cart-animated" data-product-id="${peça.id}">
                <span>Adicionar ao Carrinho</span>
                <svg class="atc-morph" viewBox="0 0 64 13">
    ...
    bindAddToCartAnimations();
}

// ---- ANIMAÇÃO GSAP ADD-TO-CART ---- //
function bindAddToCartAnimations() {
    if (typeof gsap === 'undefined' || typeof MorphSVGPlugin === 'undefined') {
        // Fallback: se GSAP não carregou, usar click simples
        document.querySelectorAll('.add-to-cart-animated').forEach(button => {
            ...
            gsap.to(button, {
                keyframes: [{
                    ...
                    onComplete() {
                        ...
                        button.style.setProperty('--atc-cart-x', '-140px');
                    }
                }, {
                    '--atc-text-o': 1,
                    '--atc-text-x': '0px',
                    ...
                    onComplete() {
                        ...
                        // Executar a lógica real de adicionar ao carrinho
                        adicionarAoCarrinho(productId);
                    }
                }]
            });
